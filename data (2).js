...
/* ============================================================
   REAL IPL 2026 SQUADS (post auction, Dec 2025)
   13-man core per team: 1 WK, 5 BAT, 3 ALL, 4 BOWL — sourced from
   published IPL 2026 retention/auction results. Fantasy points and
   credits below are still simulated (seeded per matchup/venue), but
   every name and role is a real player on that team's actual squad.
   ============================================================ */
const REAL_SQUADS = {
  csk: [
  ...
  dc: [
    ...
    { name:'Tristan Stubbs',     role:'ALL'  },
    { name:'Ajay Mandal',        role:'ALL'  },
  ...
  pbks: [
    ...
    { name:'Musheer Khan',       role:'BAT'  },
    { name:'Marcus Stoinis',     role:'ALL'  },
    { name:'Azmatullah Omarzai', role:'ALL'  },
  ...
  lsg: [
    ...
    { name:'Shahbaz Ahmed',      role:'ALL'  },
    { name:'Arshin Kulkarni',    role:'ALL'  },
...
function makeSquad(team, rng){
  ...
  // Falls back to seeded placeholder names only if a team id is ever missing
  // from REAL_SQUADS (e.g. a custom team added later) — keeps the app from
  // breaking rather than silently going blank.
  const roster = squad || ['WK','BAT','BAT','BAT','BAT','ALL','ALL','ALL','BOWL','BOWL','BOWL','BOWL','BAT']
    .map(role => ({ name: rngPick(rng, FIRST_NAMES) + ' ' + rngPick(rng, LAST_NAMES), role }));
